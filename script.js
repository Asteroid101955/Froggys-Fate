document.addEventListener('DOMContentLoaded', () => {
    ...
    const gameOverDifficultyValueElement = document.getElementById('game-over-difficulty-value');
    const winDifficultyValueElement = document.getElementById('win-difficulty-value');
    ...
    const soapItem = document.getElementById('soap-item');
    ...
    function updateCoinDisplay() {
        coins = Math.max(0, Math.min(MAX_COINS, coins));
    ...
    function updateDifficultyDisplay() {
        const translatedDifficulty = translations[currentLanguage]?.[currentDifficultyKey] || currentDifficultyKey;
        difficultyValueElement.textContent = translatedDifficulty;
        gameOverDifficultyValueElement.textContent = translatedDifficulty;
        winDifficultyValueElement.textContent = translatedDifficulty;
    }
    ...
    let isSoapDragging = false;
    let soapOffsetX, soapOffsetY;
    let activeSoapPointerId = null;
    let isSoapPointerDragging = false;
    let soapPointerOffsetX = 0;
    let soapPointerOffsetY = 0;
    let lastSoapCleanTime = 0;
    let lastSoapScrubX = null;
    let lastSoapScrubY = null;
    ...
    function triggerSoapLoss() {
        ...
        specialLossTimeout = setTimeout(() => {
            ...
            triggerGameOver(translations[currentLanguage].deathSoapEaten);
        }, 5000);
    }
    ...
    function isNearFrogBody(item) {
        return isOverlapping(item, frogBody);
    }

    function updateSoapPosition(clientX, clientY) {
        soapItem.style.left = `${clientX - soapPointerOffsetX}px`;
        soapItem.style.top = `${clientY - soapPointerOffsetY}px`;
        soapItem.style.position = 'fixed';
        soapItem.style.transform = '';

        if (isNearMouth(soapItem, frogMouth)) {
            frogMouth.classList.add('mouth-open');
        } else {
            frogMouth.classList.remove('mouth-open');
        }

        scrubSoapOnFrog(clientX, clientY);
    }

    function useSoapOnFrog() {
        cleanliness = Math.min(100, cleanliness + 30);
        updateStatsDisplay();
    }

    function resetSoapScrubState() {
        lastSoapCleanTime = 0;
        lastSoapScrubX = null;
        lastSoapScrubY = null;
    }

    function scrubSoapOnFrog(clientX, clientY) {
        if (isNearMouth(soapItem, frogMouth)) {
            return;
        }

        if (!isNearFrogBody(soapItem)) {
            lastSoapScrubX = null;
            lastSoapScrubY = null;
            return;
        }
        ...
        const movedEnough =
            lastSoapScrubX === null ||
            Math.hypot(clientX - lastSoapScrubX, clientY - lastSoapScrubY) >= 12;

        if (movedEnough && now - lastSoapCleanTime >= 120) {
            cleanliness = Math.min(100, cleanliness + 2);
            updateStatsDisplay();
            lastSoapCleanTime = now;
            lastSoapScrubX = clientX;
            lastSoapScrubY = clientY;
        }
    }

    function finishSoapDrop() {
        frogMouth.classList.remove('mouth-open');

        if (isNearMouth(soapItem, frogMouth)) {
            soapItem.style.display = 'none';
            resetSoapScrubState();
            triggerSoapLoss();
            return;
        }
    ...
    function endSoapPointerDrag() {
        if (!isSoapPointerDragging) {
            return;
        }

        isSoapPointerDragging = false;
        activeSoapPointerId = null;
        document.body.classList.remove('food-dragging');
        finishSoapDrop();
    }
    ...
    soapItem.addEventListener('dragstart', (e) => {
        isSoapDragging = true;
        resetSoapScrubState();
        soapOffsetX = e.clientX - soapItem.getBoundingClientRect().left;
        soapOffsetY = e.clientY - soapItem.getBoundingClientRect().top;
        e.dataTransfer.setDragImage(new Image(), 0, 0);
        soapItem.style.cursor = 'grabbing';
    });

    soapItem.addEventListener('pointerdown', (event) => {
        if (event.pointerType === 'mouse' || gameScreen.classList.contains('hidden') || specialLossTimeout) {
            return;
        }

        event.preventDefault();
        isSoapPointerDragging = true;
        activeSoapPointerId = event.pointerId;
        soapItem.setPointerCapture(event.pointerId);
        document.body.classList.add('food-dragging');
        resetSoapScrubState();

        const rect = soapItem.getBoundingClientRect();
        soapPointerOffsetX = event.clientX - rect.left;
        soapPointerOffsetY = event.clientY - rect.top;
        updateSoapPosition(event.clientX, event.clientY);
    });

    soapItem.addEventListener('pointermove', (event) => {
        if (activeSoapPointerId === event.pointerId && isSoapPointerDragging) {
            updateSoapPosition(event.clientX, event.clientY);
        }
    });

    soapItem.addEventListener('pointerup', (event) => {
        if (activeSoapPointerId === event.pointerId) {
            endSoapPointerDrag();
        }
    });

    soapItem.addEventListener('pointercancel', (event) => {
        if (activeSoapPointerId === event.pointerId) {
            endSoapPointerDrag();
        }
    });
    ...
    document.addEventListener('drag', (e) => {
        if (isDragging) {
        ...
        } else if (isSoapDragging) {
            soapItem.style.left = `${e.clientX - soapOffsetX}px`;
            soapItem.style.top = `${e.clientY - soapOffsetY}px`;
            soapItem.style.position = 'fixed';
            soapItem.style.transform = '';
            ...
            scrubSoapOnFrog(e.clientX, e.clientY);
        }
    });
    ...
    workButton.addEventListener('click', () => {
        coins = Math.min(MAX_COINS, coins + 10); // Earn coins, capped at 100
        updateCoinDisplay();
        
        fullness = Math.max(0, fullness - 50);
        cleanliness = Math.max(0, cleanliness - 50);
    ...
    closeTutorialButton.addEventListener('click', () => {
        ...
        difficultyDisplay.classList.remove('hidden'); // Show difficulty display
    ...
    function disableGameInteractions() {
        ...
        soapItem.draggable = false;
        soapItem.style.cursor = 'default';
        ...
        soapItem.style.pointerEvents = 'none';
    }
